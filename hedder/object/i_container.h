...
template <typename T>
concept ContainerObject = std::derived_from<T, IContainer> || requires(T t, std::vector<RenderQueueData>&queue, std::deque<std::string>&tag) 
{
    { t.SubmitDrawRequests(queue, tag) } -> std::same_as<void>;
